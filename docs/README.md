# Documentation

The website source is kept in `pages/`. This directory contains product and contributor documentation only.

## User guides

- [Getting started](getting-started.md) — install romty, create the first workspace, and use CLI commands
- [Interface](interface.md) — navigate workspaces and terminals, use scrollback and the mouse, and interpret markers
- [Runtime](runtime.md) — run agents on an always-on machine over SSH, and understand session persistence, local files, and the security boundary
- [Agent status hooks](agent-hooks.md) — install Claude Code, Codex, and OpenCode hooks and verify agent phases

## Contributor guide

- [Development](development.md) — build and test changes, preserve protocol compatibility, update the website, and publish releases
