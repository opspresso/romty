...
The workspace action menu also provides `All files`. It recursively lists every regular file in the workspace, including ignored and dot-prefixed files such as `.env`, while skipping dot-prefixed directories such as `.git`. Each level lists directories before files, with each group sorted by name, and directories start collapsed. The selected file opens read-only with syntax highlighting when its format and size are supported. Symlinks are listed neither as files nor as traversable directories. This view is also available for workspaces that are not Git repositories.

Both file views allow directory rows to take the cursor. Press `Enter` to toggle the selected directory, `←` to collapse it, or `→` to expand it. Click a directory to select and toggle it, click a file to select and open it, or use the wheel over the tree to move its selection. Collapsed directories remain collapsed when `F5` reloads the view.
...
| Key | Action |
|---|---|
| `F8` | Open actions for the selected workspace or root |
